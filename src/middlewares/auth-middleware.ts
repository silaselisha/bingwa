 ...
 * authorize th user
 */
const authMiddleware = catchAsync(async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  ...
  if (fields.length !== 2) throw new UtilsError('authorization header invalid', 401)
  ...
  const isPasswordChanged = await user?.verifyPasswordChange(decode?.iat as number)
